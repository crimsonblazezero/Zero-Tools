...
默认输出目录：`./source`（当前工作空间下的 source 目录）
...
# 运行导出脚本
python3 ~/.workbuddy/skills/wechat-article-reader/scripts/export.py "$URL" "$OUTPUT_DIR"
```
...
```bash
# 基本导出
python3 ~/.workbuddy/skills/wechat-article-reader/scripts/export.py "https://mp.weixin.qq.com/s/xxx" ~/.openclaw/workspace-qiming/source

# 指定自定义输出目录
python3 ~/.workbuddy/skills/wechat-article-reader/scripts/export.py "$URL" "/path/to/output"
```
