...
### ⚠️ 王祎个人周目标销售额必须写入（2026-08-10 更新为两人制口径）

- 王祎个人周目标 = 月目标 ÷ 4（2026-08 月目标 $112,000 → **$28,000**）
- 化一博周目标 = 月目标 ÷ 4（2026-08 月目标 $168,000 → **$42,000**）
- 整组周目标 = $70,000（$28,000 + $42,000）
- Excel中 Row2（序号100=整组）E列填$70,000；Row3（王祎个人）E列填$28,000；Row4（化一博）E列填$42,000
- ⚠️ 2026-08-10 起团队为**两人制**（王祎+化一博，乔雅静已移除），wy+hyb=全组；具体月目标以目标测算表「人员」sheet 为准
...
- type=9 附件 → ⚠️ **API 不支持附件字段提交**（2026-08-08 实测 PARAM_ERROR），改用正文 Markdown 链接
...
### 周目标销售额修正（2026-08-10 更新为两人制口径）
❌ 错误：王祎个人 `daily_tgt: None`
✅ 正确：**王祎个人周目标 = 月目标 ÷ 4**（2026-08 月目标$112,000 → **$28,000**）
✅ **化一博周目标 = 月目标 ÷ 4**（2026-08 月目标$168,000 → **$42,000**）
✅ 两人制：王祎+化一博=全组（乔雅静已移除），整组 2026-08 周目标 = **$70,000**
...
### 清货目标存量 ⚠️ 由当前库存反推
- **90-180天库龄目标存量 = 当前该段FBA库存数量 × 0.8**
- **181-270天库龄目标存量 = 当前该段FBA库存数量 × 0.5**
- **271-365天库龄目标存量 = 当前该段FBA库存数量 × 0.3**
- **366天以上库龄目标存量 = 当前该段FBA库存数量 × 0.1**
- 不是固定值，不是人工录入值，是用当前库存按比例反算（系数与 lingxing-weekly-report 新版一致）
...
# ⚠️ 2026-08-10：必须加 --to-user-ids（接收人 userId 逗号分隔），否则日志无人收到（read_number=0）
dws report entry submit \
  --template-id <模板ID> \
  --contents '<json 含 markdown 链接>' \
  --to-user-ids 01104268151043122,01231127491636542865,2212601849794046,014229691438662957,024908435521592540,0455536261847357,0501643723845167,064243141938867771,0917545705950670,101501180838205713,1352305732897533,154338120021440576,17566881508928543,223666541827618229,2260394838899755 \
  --yes
```
...
| E 本周目标销售额 | 周目标 | 整组=月目标÷4（两人制：王祎+化一博）；个人=个人月目标÷4 |
