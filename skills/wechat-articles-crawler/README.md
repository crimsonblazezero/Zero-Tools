...
本工具默认以**保守节奏**抓取，以降低触发微信公众平台限流 / 封号的风险：

- 后台 API 调用与文章下载均带「固定间隔 + 随机抖动」，避免请求节奏过于规律。
- 默认并发 3 路（旧版为 6）；遇到 `429` / `5xx` / "操作频繁" 会自动指数退避重试。
- 可用 `--safe` 进一步放慢、`--fast` 加快（风险自担），或在 `config.json` 的 `throttle` 字段长期调参。
- 时间窗（`--since` / `--until`）默认按**中国时区**对齐，避免与 UTC 差一天误判。

请勿用本工具对单一账号进行超高频、超大量的连续抓取；如确需大批量，请配合 `--safe`、合理的时间窗与人工间隔。
