...
```
[领星 ERP & AI表格] ──> [1. 填报 Excel] ──> [2. 上传钉盘取 docUrl] ──> [3. 组装 payload（附件=钉盘链接贴正文）]
                                                                                      │
                                                                                      ▼
...
> - **附件**：钉钉日志 API 不支持附件字段（type=9），Excel 以钉盘链接 Markdown 形式贴入正文文本字段，文件本体由王祎手动在钉钉客户端添加
...
在上传周会纪要文件到钉盘后，Agent需要运行此命令将文件权限授权给组员：
```bash
dws drive permission add --node <fileId_or_docUrl> --users 17566881508928543,17597998065506150 --role EDITOR -y
```
...
- `dws drive upload` 上传 Excel → 提取 `spaceId`, `fileId`, `fileName`, `fileSize`, `docUrl`
- `dws drive publish set --file-id <fileId> -y` 设置企业内公开
...
|---|-----|------|------|-------------|-------------|
| 1 | 附件 | 56 | 9 | origin | JSON 数组：`[{"spaceId":"...","fileId":"...","fileName":"...xlsx","fileSize":...,"fileType":"xlsx"}]` |
...
| 24 | 老品月库销比超过2个月的产品名称/降低库销比方案/预期需要多久完成 | 18 | 1 | markdown | 动态：ASIN对应中文品名+在库数量 |
| 25 | 超过180天的SKU数量/以及对应的货量总和/预期多久清理完毕 | 19 | 1 | markdown | 动态：SKU数/货量总和/预期60天清完 |
...
5. **附件文件本体由王祎在钉钉客户端手动添加**（API 限制，无法自动化），添加后由王祎手动转发给管理人员

### 步骤 6：六组周会纪要（维持不变，自动推送）

- 生成《六组周会会议纪要2026MMDD.xlsx》→ 上传钉盘 → 设置 EDITOR 权限 → 自动推送钉群
- 此报表**无需王祎确认**，全自动执行
