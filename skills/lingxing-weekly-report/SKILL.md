...
**⚠️ 接收人必须显式指定（2026-08-10 修复）**：dws `report entry submit` **不会自动带上模板预设接收人**，历史提交因缺 `--to-user-ids` 导致 `read_number=0`（无人收到）。提交时必须传 `--to-user-ids`，值为模板接收人清单（15 人，见下）。

**模板接收人清单（2026-08-10 用户确认，含王祎本人）**：
```
01104268151043122,01231127491636542865,2212601849794046,014229691438662957,024908435521592540,0455536261847357,0501643723845167,064243141938867771,0917545705950670,101501180838205713,1352305732897533,154338120021440576,17566881508928543,223666541827618229,2260394838899755
```
对应人员：胡佳、郑炎城、张斐、兆汇、周文君、杨桥、杨志、高应婷、王莹、陆雨阳、沈超、向家祥、王祎、汪玉琴、沈阳。
...
3. 执行提交（王祎已确认后）：
   ...
   dws report entry submit \
     --template-id 17a14a44cdee2e409b88ad14ca68d77b \
     --contents '<json>' \
     ...
     --yes
   ```
   （注意：`--contents-file` 有 cwd 解析 bug，用 `--contents` 单行 JSON 或 `--contents -` stdin；`--to-user-ids` 为接收人 userId 逗号分隔，**必填**；`--to-chat` 用户确认不需要，不传）
