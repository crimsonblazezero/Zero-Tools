...
[领星 ERP & AI表格] ──> [1. 填报 Excel] ──> [2. 导入在线表格+授权] ──> [3. 组装 payload（在线表格链接贴备注）]
                                                                                      │
                                                                                      ▼
[4. 输出关键数据 MD 给王祎审核] ──确认/执行──> [5. 提交运营周复盘日志 + 推送周会纪要（--push）]
```

> **关键闸门（2026-08-10 王祎拍板，双报表均需审核）**：
> - **每周流程**：先 `--all`（生成 Excel/在线表格 + 打印**关键数据 MD 待审核**，**不发送任何消息**）→ 王祎审核 MD → 确认后 `--all --push`（提交周报日志 + 推送周会纪要 + 转发高应婷）
> - **报表一（运营周复盘日志）**：MD 审核确认后才提交；未经确认禁止 submit
> - **报表二（六组周会纪要）**：2026-08-10 起**不再自动推送**——生成+MD 待审核，确认后 `--push` 才推送钉群
...
```bash
# 每周标准流程（2026-08-10）：
python src/weekly_report_pipeline.py --all --week-end <周六> --meeting-date <下周一>   # ① 生成+MD 待审核（不发送）
python src/weekly_report_pipeline.py --all --week-end <周六> --meeting-date <下周一> --push  # ② 审核通过后发送

# 单独执行
python src/weekly_report_pipeline.py --report1 [--push]    # 仅报表一
python src/weekly_report_pipeline.py --report2 [--push]    # 仅报表二
...
| K | 月销售额达成率 | 公式 `=I2/H2` |
| L | 月目标毛利额 | FY2026 目标表 |
| M | 月实际毛利额 | **结算毛利 `gross_profit`（利润报表 query_order_profit_list，南京组）× 0.6**（2026-08-09 用户确认：与领星前台一致；采购与头程未完全扣减仍需打折；接口为 USD 无汇率换算）。⚠️ 三行（Row2全组/Row3王祎/Row4化一博）**均为结算×0.6**（2026-08-10 修正：原王祎行缺结算字段回退预估口径导致虚高至超全组） |
| N | 毛利额达成率 | 公式 `=M2/L2` |
...
### 步骤 4：王祎检查确认（强制闸门，2026-08-08 双报表 2026-08-10）

- **输出关键数据 MD 给王祎审核**（pipeline `--all` 自动打印）：周销量/周销售额/周AcoAs、月目标/实际销售额及达成率、月目标/实际毛利额及完成率、库销比/库龄、超180天SKU、本周重点任务/下周计划
- **必须等王祎回复"确认/执行"后**，才可执行发送（`--push`：提交日志 + 推送周会纪要 + 转发高应婷）
- 若王祎指出数据异常 → 先排查修正，再重新生成展示，直到确认
...
### 步骤 6：六组周会纪要（2026-08-10 起需审核后推送）

- 生成《六组周会会议纪要2026MMDD.xlsx》→ 导入在线表格 + 授予 EDITOR 权限 → **打印关键数据 MD 待审核**（不自动推送）
- **王祎审核 MD 确认后**，`--push` 才推送钉群（卡片带可编辑链接）；2026-08-10 起**不再自动推送**
...
> **2026-08-10 修正**：8月毛利目标 40,000 → **18,000**、9月 40,000 → **24,000**（原误 40,000）；团队改**两人制**（王祎+化一博，乔雅静已移除），王祎/化一博拆分按目标测算表「人员」sheet 实际值（**王祎+化一博 = 全组**）。
