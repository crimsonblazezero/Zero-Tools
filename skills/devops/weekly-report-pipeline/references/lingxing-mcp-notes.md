...
| 指标 | 公式 | 说明 |
|------|------|------|
| 毛利额 | 领星 predict_gross_profit × **0.6** ÷ 6.8109 | 部分成本未扣减的修正系数（周报/周会纪要统一口径） |
| ACoS % | 直接取领星广告报表 | 不从利润报表算 |
| 周目标销售额 | 月目标 ÷ 4 | 固定分配 |
| 90-180天库存目标 | 当前FBA库存 × **0.8** | 按当前存量推算 |
| 181-270天库存目标 | 当前FBA库存 × **0.5** | 按当前存量推算 |
| 271-365天库存目标 | 当前FBA库存 × **0.3** | 按当前存量推算 |
| 366天以上库存目标 | 当前FBA库存 × **0.1** | 按当前存量推算 |
