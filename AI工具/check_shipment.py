...
def perform_check(zip_path):
    ...
    try:
        ...
        # Locate Overweight label PDF
        # 定位超重标签
        overweight_files = [f for f in files if f.endswith('.pdf') and any(k in f.lower() for k in ['超重', 'heavy', 'team_lift', 'teamlift', 'team lift', 'overweight'])]
        ...
        # Clean zip name from shipment ID to avoid false positives (e.g., "FR" in "FBA19HGCHFRZ")
        zip_name_clean = zip_name.lower()
        if shipment_id_from_zip:
            zip_name_clean = zip_name_clean.replace(shipment_id_from_zip.lower(), "")
            
        europe_keywords = ['德国', 'de站', '法国', 'fr站', '意大利', 'it站', '西班牙', 'es站', '英国', 'uk站', '欧洲', 'germany', 'france', 'italy', 'spain', 'united kingdom', 'europe']
        is_europe = any(k in zip_name_clean for k in europe_keywords) or \
        ...
        has_overweight_boxes = len(weight_warnings) > 0
        missing_overweight_label = False
        
        if has_overweight_boxes:
            if is_heavy_required_market:
                if not overweight_files:
                    overweight_status = "❌ Missing / 缺失 (单箱超过15KG，且压缩包内缺少超重标签PDF)"
                    missing_overweight_label = True
                else:
                    overweight_status = f"Pass / 通过 (已包含: {overweight_files[0]})"
            else:
                overweight_status = "Not Required / 无需校验 (非超重强制贴标站点)"
        ...
        print(f"- 超重标签状态 / Overweight Label: {overweight_status}")
        ...
        has_errors = any(
            ...
            for res in detail_results
        ) or not is_id_ok or not box_totals_ok or not warehouse_match or (is_europe and not gpsr_files) or missing_overweight_label
