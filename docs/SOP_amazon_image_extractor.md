...
### 第二步：运行提取脚本
在脚本所在路径下，您有以下两种方式启动程序：

*   **方式一（推荐 - 极简双击）：双击运行批处理文件**
    直接在文件夹中双击运行项目根目录下的 [run.bat](file:///d:/KovaScape%20Tools/run.bat)。
    *(此方式会自动检测并调用本地虚拟环境启动，且在运行结束或遇到报错时会自动暂停窗口而不会“闪退”，非常适合运营直接使用)*
